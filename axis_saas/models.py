...
class Student(models.Model):
    ...
    father_name = models.CharField(max_length=150, db_index=True)
    father_cnic = models.CharField(max_length=15, help_text="35202-XXXXXXX-X", db_index=True)
    parent_mobile = models.CharField(max_length=15, db_index=True)
...
# ========== SCHEDULE LABEL (FIXED REUSABLE LABELS FOR DAY SCHEDULES) ==========

class ScheduleLabel(models.Model):
    """
    Reusable fixed label used to group day-schedules (e.g. 'Senior', 'Junior',
    'Morning Shift'). Users pick a label from this table instead of free-typing.
    DaySchedule.label continues to store the label text (CharField) so existing
    rows stay valid even if a ScheduleLabel is later deleted.
    """
